...
class ModalPresenter : Subscriber {
    ...
    private func presentFaq(articleId: String) {
        ...
        #if Debug || Testflight
            vc = BRWebViewController(bundleName: "open-support-staging", mountPoint: "/?id=\(articleId)", walletManager: walletManager)
        #else
            vc = BRWebViewController(bundleName: "open-support", mountPoint: "/?id=\(articleId)", walletManager: walletManager)
        #endif
        vc.startServer()
        vc.preload()
        presentingViewController?.present(vc, animated: true, completion: {})
    ...
    private func presentBuyController(_ mountPoint: String) {
        ...
        let vc: BRWebViewController
        #if Debug || Testflight
            vc = BRWebViewController(bundleName: "open-buy-staging", mountPoint: mountPoint, walletManager: walletManager)
        #else
            vc = BRWebViewController(bundleName: "open-buy", mountPoint: mountPoint, walletManager: walletManager)
        #endif
        vc.startServer()
        vc.preload()
