...
class ModalPresenter : Subscriber {
    ...
    private func presentFaq(articleId: String? = nil) {
        ...
        #if Debug || Testflight
            vc = BRWebViewController(bundleName: "open-support-staging", mountPoint: mountPoint, walletManager: walletManager)
        #else
            vc = BRWebViewController(bundleName: "open-support", mountPoint: mountPoint, walletManager: walletManager)
        #endif
        vc.startServer()
        vc.preload()
        topViewController?.present(vc, animated: true, completion: {})
