...
class ModalPresenter : Subscriber {
    ...
    private func presentFaq() {
        ...
        #if Debug || Testflight
            vc = BRWebViewController(bundleName: "open-support-staging", mountPoint: "/", walletManager: walletManager)
        #else
            vc = BRWebViewController(bundleName: "open-support", mountPoint: "/", walletManager: walletManager)
        #endif
        vc.startServer()
        vc.preload()
        presentingViewController?.present(vc, animated: true, completion: {})
