...
class ModalPresenter : Subscriber {
    ...
    private func presentBuyController(_ mountPoint: String) {
        ...
        #if Debug || Testflight
            vc = BRWebViewController(bundleName: "open-buy-staging", mountPoint: mountPoint, walletManager: walletManager, store: store, apiClient: apiClient)
        #else
            vc = BRWebViewController(bundleName: "open-buy", mountPoint: mountPoint, walletManager: walletManager, store: store, apiClient: apiClient)
        #endif
        vc.startServer()
        vc.preload()
