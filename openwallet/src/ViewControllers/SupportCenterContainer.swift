...
class SupportCenterContainer : UIViewController {
    ...
    init(walletManager: WalletManager) {
        ...
        #if Debug || Testflight
            webView = BRWebViewController(bundleName: "open-support-staging", mountPoint: mountPoint, walletManager: walletManager)
        #else
            webView = BRWebViewController(bundleName: "open-support", mountPoint: mountPoint, walletManager: walletManager)
        #endif
        webView.startServer()
        webView.preload()
